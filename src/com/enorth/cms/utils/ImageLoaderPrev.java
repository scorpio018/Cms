...
public class ImageLoaderPrev {
	...
	private static ImageLoaderPrev mInstance;
	 ...
	 * @return
	 */
	public static ImageLoaderPrev getInstance() {

		if (mInstance == null) {
			synchronized (ImageLoaderPrev.class) {
				if (mInstance == null) {
					mInstance = new ImageLoaderPrev(1, Type.LIFO);
				}
			}
		}
		return mInstance;
	...
	private ImageLoaderPrev(int threadCount, Type type) {
...
//						imageView.setEnabled(true);
		...
		if (bm != null) {
			...
			addTask(new Runnable() {
				...
				public void run() {
					ImgBeanHolder holder = new ImgBeanHolder();
					holder.bitmap = getBitmapSaveLruCache(path, imageView);
					holder.imageView = imageView;
					holder.path = path;
					Message message = Message.obtain();
					message.obj = holder;
	 ...
	 * @return
	 */
	public Bitmap getBitmapSaveLruCache(String path, ImageView imageView) {
		Bitmap bm = getBitmap(path, imageView);
	 ...
	 * @return
	 */
	public static ImageLoaderPrev getInstance(int threadCount, Type type) {

		if (mInstance == null) {
			synchronized (ImageLoaderPrev.class) {
				if (mInstance == null) {
					mInstance = new ImageLoaderPrev(threadCount, type);
				}
			}
		}
		return mInstance;
