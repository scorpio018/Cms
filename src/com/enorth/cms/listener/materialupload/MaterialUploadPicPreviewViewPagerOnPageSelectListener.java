...
public class MaterialUploadPicPreviewViewPagerOnPageSelectListener implements OnPageChangeListener {
	...
	public MaterialUploadPicPreviewViewPagerOnPageSelectListener(MaterialUploadPicPreviewActivity activity/*List<View> items, List<String> imgDatas*/) {
	...
	public void onPageSelected(int position) {
		for (int i = 0; i < size; i++) {
			...
			if ((position - 1) <= i && i <= (position + 1)) {
				ImageLoader.getInstance().loadImageUrl(imgDatas.get(i), imageView);
			} else {
