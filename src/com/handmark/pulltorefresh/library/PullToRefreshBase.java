...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	final void setState(State state, final boolean... params) {
		...
		switch (mState) {
			case RESET:
				// 在重置时，可以释放事件阻断
				Log.e("PullOnRefreshBase", "RESET");
				this.getParent().requestDisallowInterceptTouchEvent(false);
				onReset();
				break;
			case PULL_TO_REFRESH:
				// 在下拉并没有到释放刷新的位置时，要进行事件阻断
				Log.e("PullOnRefreshBase", "PULL_TO_REFRESH");
				this.getParent().requestDisallowInterceptTouchEvent(true);
				onPullToRefresh();
				break;
			case RELEASE_TO_REFRESH:
				// 在下拉并到了释放刷新的位置时，要进行事件阻断
				Log.e("PullOnRefreshBase", "RELEASE_TO_REFRESH");
				this.getParent().requestDisallowInterceptTouchEvent(true);
				onReleaseToRefresh();
				break;
			case REFRESHING:
				// 在正在刷新时，可以释放事件阻断
				Log.e("PullOnRefreshBase", "REFRESHING");
				this.getParent().requestDisallowInterceptTouchEvent(false);
			case MANUAL_REFRESHING:
				// 在调用setRefreshing时，可以释放事件阻断
				Log.e("PullOnRefreshBase", "MANUAL_REFRESHING");
				this.getParent().requestDisallowInterceptTouchEvent(false);
			...
			case OVERSCROLLING:
				// NO-OP
				// 在正在滚动时，要进行事件阻断
				Log.e("PullOnRefreshBase", "OVERSCROLLING");
				this.getParent().requestDisallowInterceptTouchEvent(false);
				break;
		}
